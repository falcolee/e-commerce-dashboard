...
const Categories = () => {
  ...
  const handleDelete = async (id: number) => {
    Modal.confirm({
      ...
      content: 'Are you sure you want to delete this category? Categories with children cannot be deleted.',
      okText: 'Delete',
      okType: 'danger',
      onOk: async () => {
        try {
          ...
          const apiError = (error as { response?: { data?: { message?: string } } })?.response?.data?.message;
          message.error(apiError || 'Failed to delete category');
        }
      },
    });
  };
  ...
  const handleSubmit = async (values: CategoryFormValues) => {
    try {
      ...
      const apiError = (error as { response?: { data?: { message?: string } } })?.response?.data?.message;
      message.error(apiError || 'Failed to save category');
    }
  };
