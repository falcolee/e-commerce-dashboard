...
const PageCategories = () => {
  ...
  const fetchCategories = async () => {
    ...
    try {
      ...
      message.error(apiError || 'Failed to load page categories');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleImageChange = async ({ file }: UploadChangeParam<UploadFile>) => {
    ...
    try {
      ...
      message.success('Page category image uploaded successfully');
    } catch (error) {
      message.error('Failed to upload page category image');
    } finally {
  ...
  const handleDelete = async (id: number) => {
    Modal.confirm({
      ...
      content: 'Are you sure you want to delete this page category? Categories with children cannot be deleted.',
      okText: 'Delete',
      okType: 'danger',
      onOk: async () => {
        try {
          await api.taxonomies.categories.delete(id);
          message.success('Page category deleted successfully');
          fetchCategories();
        } catch (error: unknown) {
          const apiError = (error as { response?: { data?: { message?: string } } })?.response?.data?.message;
          message.error(apiError || 'Failed to delete page category');
        }
      },
    });
  };
  ...
  const handleSubmit = async (values: CategoryFormValues) => {
    try {
      ...
      if (editingCategory) {
        ...
        message.success('Page category created successfully');
      }
      setModalVisible(false);
      fetchCategories();
    } catch (error: unknown) {
      const apiError = (error as { response?: { data?: { message?: string } } })?.response?.data?.message;
      message.error(apiError || 'Failed to save page category');
    }
  };
  ...
  return (
    <div>
      ...
      <Modal
        title={editingCategory ? 'Edit Page Category' : 'Create Page Category'}
        open={modalVisible}
